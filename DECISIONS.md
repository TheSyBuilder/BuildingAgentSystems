...
- **2026-08-04 — Teach run state, sessions, memory, and durable work as separate lifetimes.** Distinguishing what survives one execution, later turns, future tasks, and process loss makes persistence an explicit product decision instead of one vague “memory” feature.
- **2026-08-05 — Store the architecture canvas as the canonical 11-field blueprint draft plus a working title.** Keeping the versioned device-local shape aligned with the textual canvas lets later blueprint export consume the reader’s architecture without translation or account state.
- **2026-08-06 — Keep observable issues and hand-labeled triage expectations in one versioned, immutable fixture envelope.** A single source of truth keeps future tool reads and evaluations coherent, while trace issues 91, 133, and 184 remain aligned with the shipped simulator.
